...
ggplot(TP_data, aes(x = treatment, y = flower_mass, fill = treatment)) +
  ...
  labs(x = "Treatment", y = "Flowerhead Mass") +
  ggtitle("Trifolium | Flowerhead Mass: Pollination excluded (Bagged) < Open",
...
# Create TPS_data, TPS_open and TPS_bagged dataframes for removing outliers
TPS_data <- TP_data
TPS_open <- TP_data %>%
  filter(treatment == "Open")
TPS_bagged <- TP_data %>%
...
ggplot(TP_data, aes(x = treatment, y = seed_mass, fill = treatment)) +
  ...
  geom_jitter(color="black", size=0.6, alpha=0.9) +
  labs(x = "Treatment", y = "Seed Mass") +
  ggtitle("Trifolium | Seed Mass: Pollination excluded (Bagged) < Open",
