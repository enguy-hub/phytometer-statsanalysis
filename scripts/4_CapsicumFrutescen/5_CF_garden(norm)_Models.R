...
smmd.lm.s9 <- lm(seedmass_meandiff ~ flo_abundance, data=CF_data)
...
smmo.lm.s9 <- lm(seedmass_meanopen ~ flo_abundance, data=CF_data)
...
summary(step.smmo.lm0) # Adj-R2: 0.88; p: 0.00193


# Check model's call
step.smmo.lm0$call # ~ lux + temp + pol_abundance + pol_shannon + flo_richness + flo_shannon
...
summary(step.rmd.lm0) # Adj-R2: 0.4953; p: 0.0468


# Check model's call 
step.rmd.lm0$call # ~ lux + imperv500 + pol_abundance + flo_abundance 
...
summary(step.rmo.lm0) # Adj-R2: 0.4366; p: 0.022
...
mpsmo.lm.s8 <- lm(mass_pseed_meanopen ~ pol_shannon, data=CF_data)
summary(mpsmo.lm.s8) # RSE: 0.00082 ; Adj-R2: -0.054 ; p: 0.5507 *

mpsmo.lm.s9 <- lm(mass_pseed_meanopen ~ flo_abundance, data=CF_data)
summary(mpsmo.lm.s9) # RSE: 0.00081 ; Adj-R2: -0.038 ; p: 0.542 *
...
# -- Create multiple regression lm() models ----
mpsmo.lm0 <- lm(mass_pseed_meanopen ~ lux + imperv100 + # temp + 
                pol_abundance + pol_shannon + flo_richness + # pol_richness + 
                flo_abundance + flo_shannon, data=CF_data)
summary(mpsmo.lm0) # Adj-R2: -0.649; p: 0.959


# Best model with stepAIC()
step.mpsmo.lm0 <- MASS::stepAIC(mpsmo.lm0, direction="both", trace=F)
summary(step.mpsmo.lm0) # Adj-R2: 0.291; p: 0.155


# Check model's call
step.mpsmo.lm0$call # ~ lux + imperv100 + pol_shannon + flo_shannon
...
# ~ lux + imperv500 + pol_abundance + flo_abundance
# Adj-R2: 0.495; p: 0.047
...
summary(rmd.lm.s3) 
# ratio_meandiff ~ imperv200; Adj-R2: 0.26; p: 0.043

summary(rmd.lm.s4)
# ratio_meandiff ~ imperv500; Adj-R2: 0.4394; p: 0.008 => Best

summary(rmd.lm.s5) 
# ratio_meandiff ~ imperv1000 ; Adj-R2: 0.28; p: 0.035
