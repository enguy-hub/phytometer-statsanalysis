...
#admfs.lm.init.trans <- MASS::stepAIC(admfs.lm.init.trans, direction = "both", trace=F)
#summ(admfs.lm.init.trans, digits= 4) # Adj-R2: 0.4379; p: 0.0225

# Check model$call
admfs.lm.init.trans$call # ~ sq.imperv100 + sq.pol_abundance + sq.flo_abundance + sq.flo_richness
...
# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.lm.init.trans) # p: 0.0281 --> NOT Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.lm.init.trans) # p: 0.572 --> Consecutive errors are independent of each other
...
# Initial model: ~ sq.imperv100 + sq.pol_abundance + sq.flo_abundance + sq.flo_richness
summ(admfs.lm.init.trans, digits=4) # Adj-R2: 0.41; p: 0.0819
...
summ(admfs.lm.inter.trans, digits=4) # Adj-R2: 0.7273; p: 0.0204

# Check model$call
admfs.lm.inter.trans$call # ~ sq.pol_abundance * sq.flo_richness * sq.imperv100 + sq.flo_abundance

# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.lm.inter.trans) # p: 0.0077 --> NOT Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.lm.inter.trans) # p: 0.33 --> Consecutive errors are independent of each other
...
# Initial model: ~ sq.pol_abundance * sq.flo_richness * sq.imperv100 + sq.flo_abundance
summ(admfs.lm.inter.trans, digits=4) # Adj-R2: 0.7273; p: 0.0204
...
# Initial (trans) model: ~ sq.imperv100 + sq.pol_abundance + sq.flo_abundance + sq.flo_richness
summ(admfs.lm.init.trans, digits=4, center=T) # Adj-R2: 0.41; p: 0.082

# Initial (trans) model: ~ sq.pol_abundance * sq.flo_richness * sq.imperv100 + sq.flo_abundance
summ(admfs.lm.inter.trans, digits=4, center=T) # Adj-R2: 0.73; p: 0.02
...
plot_model(admfs.lm.inter, type="pred", terms='imperv100', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Impervious Surface",
           axis.title=c("impervious surface of 100m buffer [%]", "fitted values | dry mass per fertile seed [g]"))
...
plot_model(admfs.lm.inter, type="pred", terms='flo_abundance', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Floral Abundance",
           axis.title=c("floral abundance", "fitted values | dry mass per fertile seed [g]"))
...
p_admfs.inter_polabu.floric <- 
  plot_model(admfs.lm.inter, type="pred", line.size=1.3,
             ...
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Pollinator Abundance
                    \nInteraction: Floral Richness",
             axis.title=c("pollinator abundance", "fitted values | dry mass per fertile seed [g]"),
             legend.title="floral\nrichness")
p_admfs.inter_polabu.floric
...
p_admfs.inter_floric.polabu <- 
  plot_model(admfs.lm.inter, type="pred", line.size=1.3,
             ...
             title="Ranunculus Acris | \nDry MassPer Fertile Seed\nvs Floral Richness
                    \nInteraction: Pollinator Abundance",
             axis.title=c("floral richness", "fitted values | dry mass per fertile seed [g]"),
...
# Table with estimated coefficients of predictors and their confidence intervals
summ(admfs.lm.inter.trans, confint=T, digits=4, ci.width=.95, center=T)
# Call: ~ sq.pol_abundance * sq.flo_richness * sq.imperv100 + sq.flo_abundance
# Adj-R2: 0.7273; p: 0.0204

# Plot how predictor 'sq.imperv100' is related to response var
plot_model(admfs.lm.inter.trans, type="pred", terms='sq.imperv100', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Impervious Surface (^2)",
           axis.title=c("impervious surface of 100m buffer (^2) [%]", "fitted values | dry mass per fertile seed [g]"))

# Plot how predictor 'sq.flo_abundance' is related to response var
plot_model(admfs.lm.inter.trans, type="pred", terms='sq.flo_abundance', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Floral Abundance (^2)",
           axis.title=c("floral abundance (^2)", "fitted values | dry mass per fertile seed [g]"))

# Plot how 'sq.pol_abundance * sq.flo_richness' is related to the fitted values of the response var
p_admfs.inter.trans_polabu.floric <- 
  plot_model(admfs.lm.inter.trans, type="pred", line.size=1.3,
             terms=c("sq.pol_abundance", "sq.flo_richness"), # [1.16, 1.72]
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Pollinator Abundance (^2)
                    \nInteraction: Floral Richness (^2)",
             axis.title=c("pollinator abundance (^2)", "fitted values | dry mass per fertile seed [g]"),
             legend.title="floral\nrichness (^2)")
p_admfs.inter.trans_polabu.floric

p_admfs.inter.trans_floric.polabu <- 
  plot_model(admfs.lm.inter.trans, type="pred", line.size=1.3,
             terms=c("sq.flo_richness", "sq.pol_abundance"), # [0.52, 0.64]
             title="Ranunculus Acris | \nDry MassPer Fertile Seed\nvs Floral Richness (^2)
                    \nInteraction: Pollinator Abundance (^2)",
             axis.title=c("floral richness (^2)", "fitted values | dry mass per fertile seed [g]"),
             legend.title="pollinator\nabundance (^2)")    
p_admfs.inter.trans_floric.polabu

gridExtra::grid.arrange(p_admfs.inter.trans_polabu.floric, p_admfs.inter.trans_floric.polabu, ncol=2)
...
# ----------- avgdrymass_ferseed & transformed predictor vars --------------

# -- Check correlation of dependent and independent vars again ----
admfs.t._vars <- c(2,3,4,5,6,7,8,10,11,12,14,15,16)
admfs.t._corr <- RA_admfs[,admfs.t._vars]
chart.Correlation(admfs.t._corr, histogram=T)


# -- Create initial multiple regression lm() model ----
admfs.t.lm0 <- lm(avgdrymass_ferseed ~ lux + imperv100 + # temp +
                  pol_abundance.yj + flo_abundance.yj + pol_richness + pol_shannon + flo_shannon + 
                  flo_richness, data=RA_admfs)
summ(admfs.t.lm0, digits=4) # Adj-R2: -0.0002; p: 0.5189


# ---- Create initial model with stepAIC() ----
admfs.t.lm.init <- MASS::stepAIC(admfs.t.lm0, direction="both", trace=F)
summ(admfs.t.lm.init, digits=4, center=T) # Adj-R2: 0.3324; p: 0.0533

# Check model$call
admfs.t.lm.init$call # ~ imperv100 + pol_richness
...
# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.t.lm.init) # p: 0.297 --> Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.t.lm.init) # p: 0.356 --> Consecutive errors are independent of each other

# Shapiro test
shapiro.test(residuals(admfs.t.lm.init)) # p: 0.4081 --> Residuals ARE norm-dist

# Check qqplot to see if residuals of fitted values of the model is normally distributed
qqnorm(residuals(admfs.t.lm.init))
qqline(residuals(admfs.t.lm.init))

# Check residual plot: Fitted values vs Residual (actual - fitted values)
residualPlots(admfs.t.lm.init, type="rstandard") # curve --> slight non-linearity

# Check CERES plot
ceresPlots(admfs.t.lm.init)

# Initial model: ~ imperv100 + pol_richness + flo_shannon
summ(admfs.t.lm.init, digits=4) # Adj-R2: 0.4177; p: 0.0498


# ---- Create interaction model ----

# Create interaction model (from initial model) using stepAIC()
admfs.t.lm.inter <- stepAIC(admfs.t.lm.init, ~.^2, trace=F)
summ(admfs.t.lm.inter, digits=4, center=T) # Adj-R2: 0.5677; p: 0.0139

# Check model$call
admfs.t.lm.inter$call # ~ imperv100 * flo_shannon

# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.t.lm.inter) # p: 0.493 --> Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.t.lm.inter) # p: 0.15 --> Consecutive errors are independent of each other

# Shapiro test
shapiro.test(residuals(admfs.t.lm.inter)) # p: 0.6 --> Residuals ARE norm-dist

# Check qqplot to see if residuals of fitted values of the model is normally distributed
qqnorm(residuals(admfs.t.lm.inter))
qqline(residuals(admfs.t.lm.inter))

# Check residual plot: Fitted values vs Residual (actual - fitted values)
residualPlots(admfs.t.lm.inter, type = "rstandard") # curve --> slight non-linearity

# Initial model: ~ imperv100 * flo_shannon
summ(admfs.t.lm.inter, digits=4, center=T) # Adj-R2: 0.5677; p: 0.0139
...
# Create transformed model
admfs.t.lm.init.trans <- lm(avgdrymass_ferseed ~ sq.imperv100 + sq.pol_richness + 
                            sq.flo_shannon, data=RA_admfs)
summ(admfs.t.lm.init.trans, digits=4, center=T) # Adj-R2: 0.4404; p: 0.0421

# Check model$call
admfs.t.lm.init.trans$call # ~ sq.imperv100 + sq.pol_richness + sq.flo_shannon

# Check for multi-collinerity: For all vars, less than 3 is good
vif(admfs.t.lm.init.trans) %>% 
  knitr::kable() # All < 3: Pass

# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.t.lm.init.trans) # p: 0.2374 --> Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.t.lm.init.trans) # p: 0.328 --> Consecutive errors are independent of each other

# Shapiro test
shapiro.test(residuals(admfs.t.lm.init.trans)) # p: 0.4844 --> Residuals ARE norm-dist

# Check qqplot to see if residuals of fitted values of the model is normally distributed
qqnorm(residuals(admfs.t.lm.init.trans))
qqline(residuals(admfs.t.lm.init.trans))

# Check residual plot: Fitted values vs Residual (actual - fitted values)
residualPlots(admfs.t.lm.init.trans, type = "rstandard") # curve --> slight non-linearity

# Check CERES plot
ceresPlots(admfs.t.lm.init.trans)

# Initial model: ~ sq.imperv100 + sq.pol_richness + sq.flo_shannon
summ(admfs.t.lm.init.trans, digits=4) # Adj-R2: 0.41; p: 0.0819


# ---- Create interaction (transformed) model ----

# Add interaction to transformed model
admfs.t.lm.inter.trans <- stepAIC(admfs.t.lm.init.trans, ~.^2, trace=F)
summ(admfs.t.lm.inter.trans, digits=4) # Adj-R2: 0.5356; p: 0.0190

# Check model$call
admfs.t.lm.inter.trans$call # ~ sq.imperv100 * sq.flo_shannon

# Test constant variance (homoscedasticity) of errors (> 0.05 = pass):
ncvTest(admfs.t.lm.inter.trans) # p: 0.2965 --> NOT Pass

# Auto-correlated Errors test - H0: consecutive errors are not correlated (p > 0.05 = pass)
durbinWatsonTest(admfs.t.lm.inter.trans) # p: 0.108 --> Consecutive errors are independent of each other

# Shapiro test
shapiro.test(residuals(admfs.t.lm.inter.trans)) # p: 0.29 --> Residuals ARE norm-dist

# Check qqplot to see if residuals of fitted values of the model is normally distributed
qqnorm(residuals(admfs.t.lm.inter.trans))
qqline(residuals(admfs.t.lm.inter.trans))

# Check residual plot: Fitted values vs Residual (actual - fitted values)
residualPlots(admfs.t.lm.inter.trans, type = "rstandard") # curve --> slight non-linearity

# Initial model: ~ sq.imperv100 * sq.flo_shannon
summ(admfs.t.lm.inter.trans, digits=4) # Adj-R2: 0.5356; p: 0.0190
...
# Initial model
admfs.t_init_fitval <- predict(admfs.t.lm.init, RA_admfs, interval="confidence") %>%
  data.frame() 
admfs.t_g1 <- fitted_vs_actual(admfs.t_init_fitval, RA_admfs$avgdrymass_ferseed, 
                                "avgdrymass_ferseed - Initial Model")

# Interaction model
admfs.t_inter_fitval <- predict(admfs.t.lm.inter, RA_admfs, interval="confidence") %>%
  data.frame() 
admfs.t_g2 <- fitted_vs_actual(admfs.t_inter_fitval, RA_admfs$avgdrymass_ferseed, 
                                "avgdrymass_ferseed - Interaction Model")

# Initial (trans) model
admfs.t_init_trans_fitval <- predict(admfs.t.lm.init.trans, RA_admfs, interval="confidence") %>%
  data.frame()
admfs.t_g3 <- fitted_vs_actual(admfs.t_init_trans_fitval, RA_admfs$avgdrymass_ferseed, 
                                "avgdrymass_ferseed - Initial (Transformed) Model")

# Interaction (trans) model
admfs.t_inter_trans_fitval <- predict(admfs.t.lm.inter.trans, RA_admfs, interval="confidence") %>%
  data.frame()
admfs.t_g4 <- fitted_vs_actual(admfs.t_inter_trans_fitval, RA_admfs$avgdrymass_ferseed, 
                                "avgdrymass_ferseed - Interaction (Transformed) Model")

# Plot grid: old model vs new 'non-linear transformed' model
gridExtra::grid.arrange(admfs.t_g1,admfs.t_g2,admfs.t_g3,admfs.t_g4, ncol=2)


# ---- Compare Adj-R2, p-value, and ANOVA test of the models ----

# Initial model: ~ imperv100 + pol_richness + flo_shannon
summ(admfs.t.lm.init, digits=4, center=T) # Adj-R2: 0.4177; p: 0.0498

# Interaction model: ~ imperv100 * flo_shannon
summ(admfs.t.lm.inter, digits=4, center=T) # Adj-R2: 0.5677; p: 0.0139

# Initial (trans) model: ~ sq.imperv100 + sq.pol_richness + sq.flo_shannon
summ(admfs.t.lm.init.trans, digits=4, center=T) # Adj-R2: 0.4404; p: 0.0421

# Initial (trans) model: ~ sq.imperv100 * sq.flo_shannon
summ(admfs.t.lm.inter.trans, digits=4, center=T) # Adj-R2: 0.5356; p: 0.0190

# Anova testing between models
anova(admfs.t.lm.init.trans, admfs.t.lm.inter.trans, test="F") # p: 9.829e-08 => Not significant
anova(admfs.t.lm.init, admfs.t.lm.inter, test="F") # p: 1.549e-07 => Not significant
...
# Table with estimated coefficients of predictors and their confidence intervals
summ(admfs.t.lm.inter, confint=T, digits=4, ci.width=.95, center=T)
# Call: ~ imperv100 * flo_shannon
# Adj-R2: 0.5677; p: 0.0139

# Plot how predictor 'imperv100' is related to response var
plot_model(admfs.t.lm.inter, type="pred", terms='imperv100', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Impervious Surface",
           axis.title=c("impervious surface of 100m buffer [%]", "fitted values | dry mass per fertile seed [g]"))

# Plot how 'imperv100 * flo_shannon' is related to the fitted values of the response var
p_admfs.t.inter_im100.flosha <- 
  plot_model(admfs.t.lm.inter, type="pred", line.size=1.3,
             terms=c("imperv100", "flo_shannon"), # [1.16, 1.72]
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Impervious Surface
                    \nInteraction: Floral Shannon Index",
             axis.title=c("impervious surface of 100m buffer [%]", "fitted values | dry mass per fertile seed [g]"),
             legend.title="floral\nshannon\nindex")
p_admfs.t.inter_im100.flosha

p_admfs.t.inter_flosha.im100 <- 
  plot_model(admfs.t.lm.inter, type="pred", line.size=1.3,
             terms=c("flo_shannon", "imperv100"), # [0.52, 0.64]
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Floral Shannon Index
                    \nInteraction: Impervious Surface",
             axis.title=c("floral shannon index", "fitted values | dry mass per fertile seed [g]"),
             legend.title="impervious\nsurface\nof 100m\nbuffer [%]")    
p_admfs.t.inter_flosha.im100

gridExtra::grid.arrange(p_admfs.t.inter_im100.flosha, p_admfs.t.inter_flosha.im100, ncol=2)


# ---- Interaction (transformed) model ----

# Table with estimated coefficients of predictors and their confidence intervals
summ(admfs.t.lm.inter.trans, confint=T, digits=4, ci.width=.95, center=T)
# Call: ~ sq.imperv100 * sq.flo_shannon
# Adj-R2: 0.5356; p: 0.0190

# Plot how predictor 'sq.imperv100' is related to response var
plot_model(admfs.t.lm.inter.trans, type="pred", terms='sq.imperv100', show.data=T, line.size=1.3,
           title="Ranunculus Acris | Dry Mass Per Fertile Seed vs Impervious Surface (^2)",
           axis.title=c("impervious surface of 100m buffer (^2) [%]", "fitted values | dry mass per fertile seed [g]"))

# Plot how 'sq.imperv100 * sq.flo_shannon' is related to the fitted values of the response var
p_admfs.t.inter.trans_im100.flosha <- 
  plot_model(admfs.t.lm.inter, type="pred", line.size=1.3,
             terms=c("imperv100", "flo_shannon"), # [1.16, 1.72]
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Impervious Surface (^2)
                    \nInteraction: Floral Shannon Index (^2)",
             axis.title=c("impervious surface of 100m buffer (^2) [%]", "fitted values | dry mass per fertile seed [g]"),
             legend.title="floral\nshannon\nindex (^2)")
p_admfs.t.inter.trans_im100.flosha

p_admfs.t.inter.trans_flosha.im100 <- 
  plot_model(admfs.t.lm.inter, type="pred", line.size=1.3,
             terms=c("flo_shannon", "imperv100"), # [0.52, 0.64]
             title="Ranunculus Acris | \nDry Mass Per Fertile Seed\nvs Floral Shannon Index (^2)
                    \nInteraction: Impervious Surface (^2)",
             axis.title=c("floral shannon index (^2)", "fitted values | dry mass per fertile seed [g]"),
             legend.title="impervious\nsurface\nof 100m\nbuffer (^2) [%]")    
p_admfs.t.inter.trans_flosha.im100

gridExtra::grid.arrange(p_admfs.t.inter.trans_im100.flosha, p_admfs.t.inter.trans_flosha.im100, ncol=2)
